...
Step 1. Find all "control" columms 
...
>Q6. Let's log transofrm this count data:

```{r}
plot(meancounts, log="xy")
```
>Q5b. You could also use the ggplot2 package to make this figure producing the plot below. What geom_?() function would you use for this plot? geom_point()
...
head(meancounts)
```
There are some "funky" log2fs values (NaN and -Inf) here that have come about whenever we have 0 mean count values. Typically we would remove these genes from any further analysis - as e can't cay anything about them if we have no data for them. 

>Q7. What is the purpose of the arr.ind argument in the which() function call above? Why would we then take the first column of the output and need to call the unique() function?

arr.ind = TRUE.
We take the first column because we care about which rows are zeros. () to avoids counting the same gene multiple times.
...
No because we are not considering statistical significance.
...
This is a main summary results figure form these kinds of studies. It is a plot of Log2-foldchange vs P-value.

```{r}
plot(res$log2FoldChange,
     res$padj)
```
Again this y-axis is highly skewed and needs a log transformation plot and we can flip the y-axis with a minus sign so it looks like every other volcano plot.
...
> Make a presentation quality ggplot version of this plot. Include clear axis labels, a clean theme, your custom colors, cut-off lines and a plot title. 
...
>Q11. Run the mapIds() function two more times to add the Entrez ID and UniProt accession and GENENAME as new columns called res$entrez, res$uniprot and res$genena
...
What known biological pathways do our differentialy expressed genes overlap with (i.e. play a role in)?
...
>Q12. Can you do the same procedure as above to plot the pathview figures for the top 2 down-reguled pathways Graft-versus-host disease" and "Type I diabetes"?
