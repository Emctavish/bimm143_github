...
> **N.B.** We never run an install cmd in a quarto code chubnk or we will end up re-installing packages many many times - which is not what we want!
...
- The **geom**emtry, the type of plot
...
```{r}
ggplot(genes) +
aes(Condition1, Condition2, col=State) + geom_point() +
  scale_colour_manual(values=c("orange", "gray", "purple"))+
...
I want to "filter" down to a subset of data. I will use the **dplyr** package to help me. 
