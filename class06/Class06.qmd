...
The `samploe()` function can be a useful starting point here:
...
generate_dna <- function(len=6) {
  sample(x=c("A","T","C","G"), size=len, replace = TRUE)
}
```
...
generate_dna <- function(len=6, fasta= TRUE) {
  ...
  ans <- sample(x=c("A","T","C","G"), size=len, replace = TRUE)
...
# The amino-acids to sample from 
  aa <- sample(x=c("A","R","N","D","C","Q","E","G","H","I","L","K","M","F","P","S","T","W","Y", "V"))
...
> Q. Use that function to generate generate random protein sequences between length 6 and 12.
...
Yes, the 9, 10, 11 and 12 amino-acid sequences are unique, as they do not share 100% identity with anything in the NCBI database. 
